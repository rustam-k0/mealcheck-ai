...
def confirmation_keyboard(lang: str, interaction_id: str) -> InlineKeyboardMarkup:
    labels=("✅ Всё так","✏️ Изменить","✖️ Отмена") if lang=="RU" else ("✅ Looks good","✏️ Change","✖️ Cancel")
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=labels[0],callback_data=f"meal:confirm:{interaction_id}")],[InlineKeyboardButton(text=labels[1],callback_data=f"meal:correct:{interaction_id}"),InlineKeyboardButton(text=labels[2],callback_data=f"meal:cancel:{interaction_id}")]])
def diary_keyboard(lang: str, interaction_id: str) -> InlineKeyboardMarkup:
    yes,no=(("💾 Сохранить","Не сохранять") if lang=="RU" else ("💾 Save","Don't save")); return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=yes,callback_data=f"diary:save:{interaction_id}"),InlineKeyboardButton(text=no,callback_data=f"diary:skip:{interaction_id}")]])
